...
public class MessageReceiver {
    ...
    public <T> T waitForMessage(Class<T> messageType, long timeout, TimeUnit unit) {
        ...
        if (message != null) {
            ...
            fail("Wait for " + messageType.getSimpleName() + " message timed out (" + waitTime + " ms).");
        }
        return message;
    }
    ...
    public <T> void checkNoMessageIsReceived(Class<T> messageType) {
        ...
        if (message != null) {
            fail("Received unexpected message " + message);
        }
    }
    ...
    private class MessageModel {
        ...
        private <T> void addMessage(T message) {
            ...
            @SuppressWarnings("unchecked") BlockingQueue<T> queue = (BlockingQueue<T>) getMessageQueue(message.getClass());
            queue.add(message);
        }
        ...
        private synchronized <T> BlockingQueue<T> getMessageQueue(Class<T> messageType) {
            ...
            @SuppressWarnings("unchecked") BlockingQueue<T> queue = (BlockingQueue<T>) messageMap.get(messageType);
            return queue;
        }
